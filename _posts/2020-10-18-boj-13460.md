...
date: 2020-10-18
categories: boj
photos: /assets/post_images/boj/boj_13460.png
tags: [ps,boj,algorithm,samsung,c++,simulation,bfs]
description: "백준 온라인 저지 13460번 - 구슬 탈출 2 C++ 풀이"
---
...
보드판 회전의 최소 횟수를 구해야 하므로 전체적인 구조는 `BFS` 로 하되 `queue` 에 넣는 요소를 보드판 그 자체로 했다. 보드판은 `string` 이 나열된 `vector` 형태이므로 이를 `key value` 로 가질 수 있는 `map` 자료구조를 활용했다. 물론 이와같은 형태는 결코 빠르지 않지만 위에서 언급했다시피 이 문제의 핵심은 실행 시간이 아닌 정확성에 있다고 판단했기 때문에 느리지만 확실한 방법을 택했다.

`queue` 에 넣는 값을 보드판 자체로 했기 때문의 보드판의 상태를 판별하는 코드의 작성이 필요했다. 이 외에도 빨간 구슬과 파란 구슬이 겹치는 경우를 처리하기 위한 코드 작성도 요구됐다.
